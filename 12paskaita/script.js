...
function createTable(){
	var eilnr = prompt("iveskite eiluciu skaiciu");
	var stulpnr = prompt("iveskite stulpeliu skaiciu");

	var tableElement = document.getElementById("myTable"); //kad nereiktu kurti daug eiluciu - susikuriam si viena

	for(var eil = 0; eil<eilnr; eil++){   //eilutes turetu nevirsyti eiluciu skaiciaus(eilnr)
		var x = tableElement.insertRow(eil); //x eilutei
		for(var stulp = 0; stulp<stulpnr; stulp++){
			var y = x.insertCell(stulp);               //y stulpeliui
			y.innerHTML = "Eilute: " +eil+ "Stulpelis:" +stulp;
		}
	}
	document.write
}